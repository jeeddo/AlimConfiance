...
export default function FilterModalMobileDevices({isFilterMobileActivated, setBtnState, ...props} : FilterModalMobileDevicesPros) {
    return ( <div className={`${isFilterMobileActivated ? 'h-[85%]' : 'h-0'} transition-all overflow-y-hidden duration-700 bg-bg md:hidden fixed bottom-0 w-full flex flex-col justify-start items-center gap-16 rounded-t-2xl xs:text-base text-sm`}>
