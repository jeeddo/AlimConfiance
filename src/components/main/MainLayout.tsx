...
export default function MainLayout() {
  ...
  const [isLoading, setLoading] = useState(false)
    ...
    const fetchFirstData = async (): Promise<void> => {
      try {
        setLoading(true)
        const api = await fetch('https://dgal.opendatasoft.com/api/explore/v2.1/catalog/datasets/export_alimconfiance/records?limit=6&where=app_libelle_activite_etablissement="Restaurants"')
        if (!api.ok) throw new Error('Failed to fetch data..')
        setFirstData((await api.json()).results.map((restaurant: Record<string, unknown>) => ({
