...
export default function useHomeLayoutState() {
  ...
  const [printRestaurantDetails, setPrintRestaurantDetails] =
  ...
  const [isFilteredRestaurantLoading, setIsFilteredRestaurantLoading] =
    useState(false)
  const [isSearchRestaurantBtnClicked, setIsSearchRestaurantBtnClicked] =
    useState(false)
  ...
  const pageCountFilteredRestaurant = useMemo(
    () =>
      filteredRestaurantCount > NB_MAX_DATA
        ? PAGE_COUNT
        : Math.ceil(filteredRestaurantCount / LIMIT),
