...
export const getRestaurant = async (
  ...
  const restaurant =
    await fetchData<Unwrap<ReturnType<typeof getRestaurant>>>(urlPath)
  if (restaurant === undefined) return []
  return restaurant.map((restaurant: Restaurant) => ({
    ...restaurant,
    inspectionDate: formatDate(restaurant.inspectionDate),
    rating: createRate(restaurant.rating as RatingsKeys)
  }))
}

export const getSpecificRestaurant = async (
  queryValue: string
): Promise<Restaurant[]> => {
  const urlPath = RESTAURANT_PATH + '/search?query=' + queryValue
  const restaurant =
    await fetchData<Unwrap<ReturnType<typeof getSpecificRestaurant>>>(urlPath)
  if (restaurant === undefined) return []
  return restaurant.map((restaurant: Restaurant) => ({
    ...restaurant,
    inspectionDate: formatDate(restaurant.inspectionDate),
...
export const getFilteredRestaurants = async (
  ...
  const urlPath =
    RESTAURANT_PATH +
    `/filter?location=${location}&hygieneLevel=${hygieneLevel}&sortFilter=${sortFilter}&limit=${limit}&offset=${offset}`
  const restaurant =
    await fetchData<Unwrap<ReturnType<typeof getFilteredRestaurants>>>(urlPath)
